...
state_dict1 = torch.load('训练参数/单模态vgg_unet第22轮.pth', map_location=torch.device('cpu')) 
mynet1.load_state_dict(state_dict1)
...
if __name__=="__main__":
    for epoch in range(10):
        ...
        for x,y in train_loder:
            ...
            optimizer.zero_grad()
            y=mynet1(x)
            y_pred_classes = torch.argmax(y, dim=1)
            y_pred=mynet(x)
    
            loss=criterion(y_pred,y_pred_classes)
            loss.backward()
            optimizer.step()
            i+=1
            if(i%5==0):
