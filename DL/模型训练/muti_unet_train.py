import sys  
sys.path.append("模型代码")
from muti_unet import Multimodal_unet
import tifffile  
import cv2
import torch
import torchvision
...
class cuntomdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label=Image.open(os.path.join(self.label_dir,self.labels[index]))
...
mynet=Multimodal_unet(n_classes)
mynet.to(device)
mynet.train()
...
if __name__=="__main__":
    for epoch in range(10):
        ...
        torch.save(mynet.state_dict(), '../训练参数/多模态unet第{}轮.pth'.format(epoch+1))