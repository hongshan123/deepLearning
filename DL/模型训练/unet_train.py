...
class cuntomdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        trans_label[np.all(label==np.array([0.0,255.0,0.0]),axis=-1)]=1#1
        trans_label[np.all(label==np.array([0.0,255.0,255.0]),axis=-1)]=2
        trans_label[np.all(label==np.array([255.0,0.0,0.0]),axis=-1)]=3#1
        trans_label[np.all(label==np.array([255.0,255.0,0.0]),axis=-1)]=4#1
        trans_label[np.all(label==np.array([255.0,255.0,255.0]),axis=-1)]=5#1
...
if __name__=="__main__":
    for epoch in range(10):
        ...
        #print("现在是第{}轮训练，loss为{}".format(epoch+1,loss))
        torch.save(mynet.state_dict(), '../训练参数/单模态unet第{}轮.pth'.format(epoch+1))